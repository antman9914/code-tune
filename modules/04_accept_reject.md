...
输出：
```
✓ 接受：{metric_name} {metric_before} → {metric_after}（+{|delta|}）
  baseline 已更新
```
...
输出：
```
✗ 拒绝：{metric_name} {metric_after}（基线：{metric_before}，delta：{delta}）
...
```json
{
  ...
  "metric_after": {metric_after 或 null},
  "delta": {delta 或 null},
  "epochs_used": {epochs_used},
  "status": "{accept / reject / error / timeout}",
...
`delta` 的计算方式：
```
direction=max：delta = metric_after - metric_before
direction=min：delta = metric_before - metric_after
error/timeout：delta = null
```
...
metric_range：
  ...
  对 loss 等无固定上界指标      → metric_range = max(有效值) - min(有效值)
  （从 experiment_log.jsonl 中所有 metric_after 不为 null 的记录中计算）
...
**此步骤处理 accept / reject 情况的 run_log，不可跳过。**
**error / timeout 情况的 run_log 由 SKILL.md Step 2 直接写入，不经过 Module 4。**
...
## 假设
**诊断依据（上一轮曲线）**：{Module 2 生成的 curve_diagnosis}
**改动描述**：{一句话}
**改动理由**：{2-3 句话}
...
**本轮曲线诊断**：{Module 3 的 curve_diagnosis（对本次实验曲线的描述）}
